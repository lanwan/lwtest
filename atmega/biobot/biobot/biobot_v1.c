...
static int8_t gActiveServoIndex1;
static int8_t gActiveServoIndex2;


#define PULSE_SLOT_TIME             100     // 100us
#define PULSE_SLOT_TIME_DIV_2       50
#define MAIN_PULSE_PERIOD           20000  // 20ms
#define MAIN_PULSE_SLOT_TIME        125 // 250us
...
ISR( TIMER0_OVF_vect )
{
	...
	if( gServos[gServoIndex].main_slot_count == 0 ){
        ...
        if( gServos[gServoIndex].current_pulse >= 1500 )
	    {	
            ...
            *(gServoPorts[gServoIndex]) |= _BV(gServoPins[gServoIndex]);
            ...
            TIFR &= ~_BV(TOV1);
            TCNT1H = 0xFF - (gServos[gServoIndex].current_pulse >> 8); // must be first
            TCNT1L = 0xFF - (gServos[gServoIndex].current_pulse & 0xFF);
            TIMSK |= _BV(TOIE1);
	    }
	    else if( gServos[gServoIndex].current_pulse >= 500 )
	    {	
            gActiveServoIndex2 = gServoIndex;       		
...
ISR( TIMER2_OVF_vect )
{
	...
	if( gServos[gActiveServoIndex2].pulse_slot_count == 0 ){
        // set low pulse
	    *(gServoPorts[gActiveServoIndex2]) &= ~_BV(gServoPins[gActiveServoIndex2]);	
        gActiveServoIndex2 = -1;		
	}
	else{
        TCNT2 = 0xFF - PULSE_SLOT_TIME; // must be first
        TIMSK |= _BV(TOIE2);
        gServos[gActiveServoIndex2].pulse_slot_count --;		
	}							
}
